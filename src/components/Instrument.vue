<template>
    <div class="instrument">
        ...
        <div class="instrument-header">
            ...
            <button @click="generateMelodies" class="regenerate">Regenerate</button>
        </div>
        ...
        <div class="code">
            <codemirror :value="fitnessFunctionString" @input="updateFitnessFunction"></codemirror>
        </div>
    </div>
</template>
...
export default {
    ...
    watch: {
        conductor(pulse) {
            // midi clocks pulse 12x per quater note
            const interval = 12;
            if (pulse % interval === 0) {
                const index = pulse / interval - 1;
                this.play(pulse, index);
            }
        }
    },
    methods: {
        ...
        play: function(pulse, pos) {
            const index = pos % this.noteStream.length;
            const prevNote = this.noteStream[index - 1];
            const note = this.noteStream[index];
            if (note === null) return;
            // If note is the same as the previous note then continue
            if (prevNote && prevNote === note) return;
            if (this.currentNote) this.output.stopNote(this.currentNote.midi); // Stop the current note if there is one
