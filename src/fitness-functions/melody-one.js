...
export default (melody, scale, utils) => {
    ...
    if (melody[0] && utils.getDistance(baseNote, melody[0].name).distance === '1P') score++;

    // If the last note is a perfect note in the scale
    if (melody[melody.length - 1] && utils.getDistance(baseNote, melody[melody.length - 1].name).interval === 'P') score++;

    // If the 1st, 5th, 9th etc. note are a perfect note
    melody.map((note, index) => {
        if (note && index % 4 === 0) {
            if (utils.getDistance(baseNote, note.name).interval === 'P') score++;
        }
    });
    ...
    melody.map((note, index) => {
        if (note && index % 4 !== 0) {
            if (utils.getDistance(baseNote, note.name).interval === 'M') score++;
        }
    });
